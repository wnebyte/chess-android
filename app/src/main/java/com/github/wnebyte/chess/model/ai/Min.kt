...
class Min : MinMax
{
    ...
    override fun generate(node: Node<State>): List<Node<State>>
    {
        ...
        for (kv in node.getData().getChessPieces().filter { kv1 -> kv1.value.color == Color.BLACK })
        {
            for (position in kv.value.legalMoves(node.getData()))
            {
                children.add(Node(State.copy(node.getData()).update(kv.value, position)))
            }
        }
        return children
    }
    ...
    override fun minmax(node: Node<State>, utilityFunction: UtilityFunction)
    {
        ...
        else
        {
            node.getData().utility =
                    node.getChildren().minBy { it.getData().utility }!!.getData().utility
        }
    }
